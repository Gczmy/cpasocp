...
class TestSets(unittest.TestCase):
    ...
    Phi = core_offline.ProximalOfflinePart(prediction_horizon, proximal_lambda, A, B, Q, R, P, Gamma_x, Gamma_u,
                                           Gamma_N).make_Phi()
    Phi_z = Phi * z0
    Phi_star = core_offline.ProximalOfflinePart(prediction_horizon, proximal_lambda, A, B, Q, R, P, Gamma_x, Gamma_u,
    ...
    def test_offline_part(self):
        P_seq, R_tilde_seq, K_seq, A_bar_seq = core_offline.ProximalOfflinePart(TestSets.prediction_horizon,
                                                                                TestSets.proximal_lambda, TestSets.A,
                                                                                TestSets.B,
                                                                                TestSets.Q, TestSets.R, TestSets.P,
                                                                                TestSets.Gamma_x, TestSets.Gamma_u,
                                                                                TestSets.Gamma_N).algorithm()
        Phi = core_offline.ProximalOfflinePart(TestSets.prediction_horizon, TestSets.proximal_lambda, TestSets.A,
                                               TestSets.B, TestSets.Q,
                                               TestSets.R, TestSets.P, TestSets.Gamma_x, TestSets.Gamma_u,
                                               TestSets.Gamma_N).make_Phi()
        Phi_z = Phi * TestSets.z0
        Phi_adj = core_offline.ProximalOfflinePart(TestSets.prediction_horizon,
                                                   TestSets.proximal_lambda, TestSets.A, TestSets.B,
                                                   TestSets.Q, TestSets.R, TestSets.P,
                                                   TestSets.Gamma_x, TestSets.Gamma_u,
                                                   TestSets.Gamma_N).make_Phi_adj()
        print(len(P_seq))
