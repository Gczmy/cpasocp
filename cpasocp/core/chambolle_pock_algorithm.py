...
def CP_precondition_for_ocp(epsilon, initial_guess_z, initial_guess_eta, T_pre, Sigma_pre, L, L_z, L_adj,
                            prediction_horizon, initial_state, state_dynamics, control_dynamics,
                            control_weight, P_seq, R_tilde_seq, K_seq, A_bar_seq, stage_state,
                            terminal_state, stage_sets, terminal_set):
    """
    ...
    for i in range(n_max):
        z_prev = z_next
        eta_prev = eta_next
        z_next = core_online.proximal_of_h_online_part_precondition(prediction_horizon=N,
                                                                    T_pre=T_pre,
                                                                    Sigma_pre=Sigma_pre,
                                                                    initial_state=x0,
                                                                    initial_guess_vector=z_prev - alpha * L_adj @ eta_prev,
