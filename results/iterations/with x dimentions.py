...
for problem_loop in range(100):
    ...
    for i in range(10):
        ...
        A = 0.02 * np.random.rand(n_x, n_x)
        B = 0.01 * np.random.rand(n_x, n_u)
    ...
    print(f"({n_x}, {iterations}), {k+1}")
    f = open("with x dimention.txt", "a")
    print(f"({n_x}, {iterations})", file=f)
    f.close()
